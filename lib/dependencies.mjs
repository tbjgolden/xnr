...
export const getDependencies = async (filename) => {
  ...
  try {
    ...
    throw new Error(`just-run couldn't read ${JSON.stringify(filename)}`);
  }
  const dependencies =
    path.extname(filename).toLowerCase() === ".tsx" ? tsx(fileString) : ts(fileString);

  return dependencies.filter(([dependency]) => {
    if (dependency.startsWith("node:")) return false;
    if (dependency === "test") return true;
