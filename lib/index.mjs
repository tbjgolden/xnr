...
const updateImports = async (ast, relativeInputFile, internalSourceFilesMap) => {
  const ensure = (dependencyPath) => {
    if (dependencyPath.startsWith(".") || dependencyPath.startsWith("/")) {
      const ext = path.extname(dependencyPath);
      const withoutExt = dependencyPath.slice(0, dependencyPath.length - ext.length);

      const internalSourceFile = internalSourceFilesMap.get(
        path.join(path.dirname(relativeInputFile), withoutExt)
      );

      return withoutExt + internalSourceFile.outputFormat;
    }
